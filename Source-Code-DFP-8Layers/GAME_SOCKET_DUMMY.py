...
import numpy as np
from array import *
from random import randrange, choice as randlist
...
class GameSocket:
    ...
    def init_bots(self):
        self.bots = []
        n_players = randlist([3,4]) # learn to play in case a player is eliminated
        for bot_id in range(2, n_players+1):
            random_num = randrange(100)
            if random_num < 33:
                random_bot = Bot1(bot_id)
            elif random_num > 66:
                random_bot = Bot2(bot_id)
            else:
                random_bot = Bot3(bot_id)
            self.bots += [random_bot]
        # use bot1(id=2), bot2(id=3), bot3(id=4)
        # self.bots = [Bot1(2), Bot2(3), Bot3(4)]  
        for bot in self.bots:  
            # at first, all bots have same position, energy as player
    ...
    def reset(self, requests):
        """
        load new game by given request: 
            [map_id (filename), pos_x, pos_y, initial_energy]
        """
        ...
        self.user.posx = self.userMatch.posx  
    ...
    def reset_map(self, id):  
        # load map info
        self.mapId = id
        self.map = json.loads(self.maps[self.mapId]) # dtype: list
        self.map = labyrinthize_map(self.map)
    ...
    def connect(self):  
        # simulate player's connect request
    ...
    def receive(self):  
        # send data to player (simulate player's receive request)
        if self.resetFlag:  
            # for the first time -> send game info
    ...
    def action_5_craft_pre(self, user):  
        # collect players who craft at current step
        user.freeCount = 0
        if self.map[user.posy][user.posx] <= 0:  
            # craft at the non-gold cell
            user.energy -= 10
            if user.energy <= 0:
    ...
    def go_to_pos(self, user):  
        # player move to cell (x,y)
...
def labyrinthize_map(map_2d: list, 
              random_values: list=[1,1,2,3,5,8,13,21]) -> list:
    map_arr = np.array(map_2d)
    random_values = np.array(random_values)
    random_probs = random_values / (np.sum(random_values)+1e-19)
    n_replacements = np.random.choice(random_values, p=random_probs)
    for replace_id in range(n_replacements):
        swap_dim = np.random.choice([0,1])
        swap_id_1 = np.random.choice(list(range(map_arr.shape[swap_dim])))
        swap_id_2 = np.random.choice(list(range(map_arr.shape[swap_dim])))
        if swap_dim == 0:
            map_arr = swap_2_rows(map_arr, swap_id_1, swap_id_2)
        elif swap_dim == 1:
            map_arr = swap_2_cols(map_arr, swap_id_1, swap_id_2)
    return map_arr.tolist()

def swap_2_cols(map_arr: np.array, col_1: int, col_2: int):
    assert len(map_arr.shape)==2, "`map_arr` is not 2D array"
    map_arr[:, [col_1, col_2]] = map_arr[:, [col_2, col_1]]
    return map_arr

def swap_2_rows(map_arr: np.array, row_1: int, row_2: int):
    assert len(map_arr.shape)==2, "`map_arr` is not 2D array"
    map_arr[[row_1, row_2], :] = map_arr[[row_2, row_1], :]
    return map_arr